...
export default class App extends React.Component{
  ...
  async handleSearch(textSearch){
    ...
    if(!data){
      ...
      const speciesApi = await fetch(data.species.url);
      const speciesData = await speciesApi.json();
  ...
  async showPokemons(limit = 20, offset = 0){
    ...
    const promises = await data.results.map(async pokemon => {
      ...
      const speciesApi = await fetch(res.species.url);
      const speciesData = await speciesApi.json();
      const description = speciesData.flavor_text_entries.find(entry => entry.language.name === 'en').flavor_text;
